...
function loadCountries() {
    ...
    for (let x = 0; x < countries.length; x++) {
        options += '<option value="' + countries[x]["name"] + '" />';
    }
...
function checkStates() {
    let currentInputC = document.getElementById('countryInput').value;
    let stateOptions = [];

    if (currentInputC.length > 0) {
        let lol = countries.find(c => c.name === currentInputC);
        if (lol != undefined) {
            let statelist = states[`${lol.code}`];
            for (let i = 0; i < statelist.length; i++) {
                stateOptions += `<option value="${statelist[i]}"/>`
            }
            document.getElementById('stateList').innerHTML = stateOptions;
        }
        else if (lol == undefined) {
